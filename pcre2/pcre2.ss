...
(defstruct pcre-regex
  (code            ; pcre2-code* (FFI pointer, GC-managed)
   ...
   jit?            ; boolean: was JIT compilation requested and succeeded?
   name-table)     ; alist ((name . group-number) ...) or '()
  final: #t)

;; Match result — immutable snapshot of one successful match
(defstruct pcre-match
  (positions   ; vector of (start . end) pairs, or #f for unset groups
   subject     ; the subject string (for substring extraction)
   name-table) ; alist from the regex (for named group access)
  final: #t)
...
(def (pcre2-build-name-table code)
  ;; Build alist mapping name → group-number from PCRE2 name table
...
(def (pcre2-compile pattern
  ...
  ;; JIT is enabled by default — improves match speed dramatically.
  (let* ((plen  (string-length pattern))
    ...
    ;; Attempt JIT compilation (gracefully skip if unsupported)
    (def jit-ok? #f)
    (when jit?
      (let ((jrc (ffi-pcre2-jit-compile code PCRE2_JIT_COMPLETE)))
        (set! jit-ok? (= jrc 0))))
    ;; Build match data and name table
...
(def (pcre2-regex pattern . flags)
  ;; Compile with keyword flags.
  ;; flags: caseless: multiline: dotall: extended: ungreedy: utf: literal:
  (def (flag-opt kw v base) (if v (bitwise-ior base kw) base))
  (let-keywords flags
    ((caseless  #f)
     (multiline #f)
     (dotall    #f)
...
(def (pcre2-do-match code md subject start options)
  ;; Run a single match. Returns rc (int), or negative on no-match/error.
  ;; Uses JIT if md is from a JIT-compiled regex (via ffi-pcre2-jit-match).
  (ffi-pcre2-match code subject (string-length subject) start options md))

(def (pcre2-capture-match rx subject start options)
  ;; Match and capture results. Returns pcre-match or #f.
  (let* ((code (pcre-regex-code rx))
         (md   (pcre-regex-match-data rx))
         (rc   (if (pcre-regex-jit? rx)
                 (ffi-pcre2-jit-match code subject (string-length subject)
                                      start options md)
                 (ffi-pcre2-match code subject (string-length subject)
                                  start options md))))
    (if (< rc 0)
      #f
      (let* ((ncap (+ (pcre-regex-capture-count rx) 1))
             (pos  (make-vector ncap #f)))
        (let loop ((i 0))
          (when (< i ncap)
            (if (ffi-pcre2-ovector-is-unset? md i)
              (vector-set! pos i #f)
              (vector-set! pos i
                (cons (ffi-pcre2-ovector-start md i)
                      (ffi-pcre2-ovector-end   md i))))
            (loop (+ i 1))))
        (make-pcre-match pos subject (pcre-regex-name-table rx))))))
...
(def (pcre2-match rx/str subject (start 0))
  ...
  (let ((rx (ensure-regex rx/str)))
    (pcre2-capture-match rx subject start
...
(def (pcre2-search rx/str subject (start 0))
  ...
  (let ((rx (ensure-regex rx/str)))
    (pcre2-capture-match rx subject start 0)))

(def (pcre2-matches? rx/str subject (start 0))
  ;; Test-only match: no capture overhead, returns boolean.
  (let* ((rx   (ensure-regex rx/str))
...
(def (pcre-match-group match (n 0))
  ;; Return group n as a string, or #f if unset.
  (let ((pair (vector-ref (pcre-match-positions match) n)))
    (and pair (substring (pcre-match-subject match)
                         (car pair) (cdr pair)))))

(def (pcre-match-named match name)
  ;; Return named capture group as a string, or #f if unset/not found.
  (let ((entry (assoc name (pcre-match-name-table match))))
    (and entry (pcre-match-group match (cdr entry)))))

(def (pcre-match-positions match (n 0))
  ;; Return (start . end) byte positions for group n, or #f if unset.
  (vector-ref (pcre-match-positions match) n))

(def (pcre-match->list match)
  ;; Return all groups as a list of strings (with #f for unset groups).
  (let* ((pos (pcre-match-positions match))
         (n   (vector-length pos)))
    (let loop ((i 0) (acc '()))
      (if (= i n)
        (reverse acc)
        (loop (+ i 1) (cons (pcre-match-group match i) acc))))))

(def (pcre-match->alist match)
  ;; Return named captures as an alist ((name . string-or-#f) ...)
  (map (lambda (entry)
         (cons (car entry) (pcre-match-named match (car entry))))
       (pcre-match-name-table match)))
...
(def (pcre2-replace rx/str subject replacement
                    (start 0)
                    extended: (extended? #f))
  ;; Replace the first occurrence of rx in subject with replacement.
  (let* ((rx   (ensure-regex rx/str))
    ...
    (unwind-protect
      (let ((rc (ffi-pcre2-do-substitute
                 ...
                 replacement rlen)))
        (if (< rc 0)
          subject  ; no match — return original
...
(def (pcre2-fold rx/str kons knil subject (start 0))
  ...
  (let ((rx (ensure-regex rx/str)))
    (let loop ((pos start) (acc knil))
      (let ((m (pcre2-capture-match rx subject pos 0)))
        (if (not m)
          acc
          (let* ((span  (vector-ref (pcre-match-positions m) 0))
                 (end   (cdr span))
                 ;; Advance past zero-length matches to avoid infinite loop
                 (next  (if (= (car span) end)
                          (+ end 1)
                          end)))
            (if (> next (string-length subject))
              (kons m acc)
...
(def (pcre2-split rx/str subject (limit #f))
  ...
  ;; limit: max number of splits (total parts = limit+1 if specified).
  (let ((rx (ensure-regex rx/str))
        (slen (string-length subject)))
    (let loop ((pos 0) (acc '()) (count 0))
      (if (and limit (>= count (- limit 1)))
        ;; Hit limit — append remainder
      ...
      (let ((m (pcre2-capture-match rx subject pos 0)))
        (if (not m)
          ...
          (let* ((span  (vector-ref (pcre-match-positions m) 0))
                 (start (car span))
                 (end   (cdr span))
                 (pre   (substring subject pos start))
                 (matched (pcre-match-group m 0))
                 (next  (if (= start end) (+ end 1) end)))
            (if (> next slen)
              (reverse (cons matched (cons pre acc)))
              (loop next (cons matched (cons pre acc))))))))))
...
(def (pcre2-quote str)
  ;; Escape all PCRE2 metacharacters in str for use as a literal pattern.
  (with-output-to-string
    (lambda ()
      (string-for-each
       (lambda (c)
         (when (string-index _meta-chars c)
           (display #\\))
         (display c))
       str))))
...
(def (pcre2-release! regex)
  ;; Explicitly release FFI resources. GC handles this automatically,
  ;; but you can call this for immediate deallocation.
  (when (pcre-regex? regex)
    (let ((code (pcre-regex-code regex))
          (md   (pcre-regex-match-data regex)))
...
(def (pcre2-pregexp-match pattern subject (start 0) (end #f))
  ;; Like pregexp-match: returns (#f or list of strings/booleans).
...
(def (pcre2-pregexp-match-positions pattern subject (start 0) (end #f))
  ;; Like pregexp-match-positions: returns (#f or list of (start . end) pairs).
  (let* ((subj   (if end (substring subject start end) (substring subject start)))
         (offset (if end start start))
         (m      (pcre2-search pattern subj 0)))
    (and m
         (let* ((pos (pcre-match-positions m))
                (n   (vector-length pos)))
           (let loop ((i 0) (acc '()))
             (if (= i n)
               (reverse acc)
               (let ((pair (vector-ref pos i)))
                 (loop (+ i 1)
                       (cons (if pair
                               (cons (+ offset (car pair))
                                     (+ offset (cdr pair)))
                               #f)
                             acc)))))))))
